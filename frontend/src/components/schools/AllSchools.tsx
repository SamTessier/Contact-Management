...
const AllSchools = (): JSX.Element => {
  ...
  const { data: schools, isLoading } = useQuery(
    ["schools"],
    async () =>
      (await (await fetch("http://${import.meta.env.VITE_API_URL}api/schools")).json()) as School[]
  );

  const { mutate: createSchool } = useMutation(async (school: School) => {
    await fetch("http://${import.meta.env.VITE_API_URL}api/schools", {
      method: "POST",
  ...
  const { mutate: updateSchool } = useMutation(async (school: School) => {
    await fetch(`http://${import.meta.env.VITE_API_URL}api/schools/${school.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(school),
    });
  ...
  const { mutate: deleteSchool } = useMutation(async (id: string) => {
    await fetch(`http://${import.meta.env.VITE_API_URL}api/schools/${id}`, {
      method: "DELETE",
    });
    queryClient.invalidateQueries(["schools"]);
  });
